...
type KVServer struct {
	...
	replyChan      map[int]chan ReplyDetail		//构造index/term双重哈希
...
func (kv *KVServer) Get(args *GetArgs, reply *GetReply) {
	...
	kv.mu.Lock()
	kv.replyChan[index]=make(chan ReplyDetail,1)
	kv.mu.Unlock()

	select {
	case ans:=<-kv.replyChan[index]:
		if term==ans.Term {
			reply.Err=ans.Err
			reply.Value=ans.Value
		}else {
		...
		case command:=<-kv.applyCh:
			...
			if command.CommandValid{
				...
				if CommandDetail, ok := kv.clientCache[op.Clientid]; ok && CommandDetail.Commandid >= op.Commandid {
					reply=CommandDetail.Reply
					kv.mu.Unlock()
					return
			 }
				switch t {
				case "Get":
					reply=kv.handleGet(term,key)
				case "Append"	:
					reply=kv.handleAppend(term,key,value)
				case "Put" :
					reply=kv.handlePut(term,key,value)
			}
				_,ok:=kv.replyChan[index]
				if ok{
					kv.replyChan[index]<-reply
				}
				kv.clientCache[op.Clientid]=CommandDetail{op.Commandid,reply}
			}
			kv.mu.Unlock()
		}
...
func (kv *KVServer) handleGet(term int,key string) ReplyDetail{
	value,ok:=kv.DataBase[key]
	if !ok {
		return ReplyDetail{ErrNoKey,"",term}
	}
	return  ReplyDetail{OK,value,term}
}

func (kv *KVServer) handlePut(term int,key string,value string) ReplyDetail{
	kv.DataBase[key]=value
	return  ReplyDetail{OK,"",term}
}

func (kv *KVServer) handleAppend(term int,key string,value string) ReplyDetail{
...
func StartKVServer(servers []*labrpc.ClientEnd, me int, persister *raft.Persister, maxraftstate int) *KVServer {
	...
	kv.replyChan=make(map[int]chan ReplyDetail)
	go kv.Listen()
	return kv
}
